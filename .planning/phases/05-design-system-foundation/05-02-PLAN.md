...
IMPORTANT: This component is `"use client"` because Motion requires browser APIs. The component does NOT use `AnimatePresence` + `exit` because Next.js App Router does not support exit animations natively (the outgoing page is unmounted before the new one renders). Entry animations only.
...
**Step 4 — Wire PageTransition into `src/app/layout.tsx`:**

Import `PageTransition` and wrap the `{children}` inside the layout body so every route gets entry animations automatically:

```tsx
import { PageTransition } from "@/components/ui/page-transition";
```

In the return JSX, wrap `{children}` with `<PageTransition>`:
...
If there is a `<MainLayout>` or other wrapper between `<body>` and `{children}`, place `<PageTransition>` inside that wrapper, wrapping `{children}` directly. The key point: `<PageTransition>` must wrap the page content that changes on navigation.

**Step 5 — Wire buttonPress micro-interaction into the shadcn/ui Button component:**

In `src/components/ui/button.tsx`, add Motion support so every Button instance gets the micro-interaction:
...
Create a `MotionButton` using `motion.create()` or by wrapping:

```tsx
import { Slot } from "@radix-ui/react-slot";

// At the bottom of the file, after the existing Button:
const MotionButton = motion.create(Button);
```

Then update the default export/re-export so that the Button component applies `{...buttonPress}` props (whileHover, whileTap, transition) by default. The simplest approach:

1. Rename the existing `Button` to `BaseButton`
2. Create a new `Button` that wraps `BaseButton` in `motion.div` with `{...buttonPress}` spread
3. Export the motion-wrapped version as `Button`

Alternatively, if the shadcn Button uses `forwardRef`, wrap it like this:

```tsx
import { motion } from "motion/react";
import { buttonPress } from "@/lib/motion";

// After the existing Button component definition:
...
Choose the approach that integrates cleanest with the existing shadcn Button code. The requirement is: at least one component in the design system uses `whileHover`/`whileTap` from the Motion library with the `buttonPress` preset from `src/lib/motion.ts`.
  </action>
  <verify>
